...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.CalculateGains=function()
		{
			...
			mult+=parseFloat(Game.prestige['Heavenly chips'])*0.02;
		...
		Game.goldenCookie.click=function()
		{
			if (Game.goldenCookie.life>0)
			{
				...
				else if (choice=='chain cookie')
				{
					...
					if ((Math.random()<0.13 || moni>=Game.cookies*0.25) && Game.goldenCookie.chain>4) Game.goldenCookie.chain=0;
					Game.Earn(moni);
				}
			}
		}
		...
		for (var i=0;i<40;i++)
		{
			Game.cookieParticles[i]={x:0,y:0,life:-1,r:0,pic:0};
		}
		...
		Game.cookieParticleAdd=function()
		{
			...
			if (Game.prefs.particles)
			{
				...
				var i=highestI;
				var x=-64;
				if (Game.LeftBackground) x=Math.floor(Math.random()*Game.LeftBackground.canvas.width);
				var y=-32;
				var me=Game.cookieParticles[i];
				me.life=0;
				me.x=x;
				...
				me.pic=Math.floor(Math.random()*10000);
			}
		}
		Game.cookieParticlesDraw=function()
		{
			for (var i in Game.cookieParticles)
			{
				var me=Game.cookieParticles[i];
				if (me.life!=-1)
				{
					...
					var opacity=1-(me.life/(Game.fps*3));
					Game.LeftBackground.globalAlpha=opacity;
					Game.LeftBackground.drawImage(Game.Assets['smallCookies.png'],(me.pic%8)*64,0,64,64,-32,-32,64,64);
		...
		Game.UpdateMenu=function()
		{
			...
			else if (Game.onMenu=='stats')
			{
				...
				if (Game.milkProgress>=3.5) milkName='orange juice';
		...
		order=100;
		new Game.Upgrade('Sextillion fingers','The mouse and cursors gain another <b>+100</b> cookies for each non-cursor object owned.<q>sometimes<br>things just<br>click</q>',500000000000000,[12,13]);
		...
		new Game.Upgrade('Orange juice','Unlocks <b>orange juice</b>, available in the menu.',120000000000,[7,8]);
		...
		*/
		...
		Game.DrawBackground=function()
		{
			...
			if (Game.prefs.particles)
			{
				...
				Game.cookieParticlesDraw();
				Game.LeftBackground.globalAlpha=1;
				
				//big cookie shine
				var s=512;
				var x=Math.floor(Game.LeftBackground.canvas.width/2);
				var y=Math.floor(Game.LeftBackground.canvas.height*0.4);
			...
			else
			{
				//big cookie shine
				var s=512;
				var x=Math.floor(Game.LeftBackground.canvas.width/2-s/2);
				var y=Math.floor(Game.LeftBackground.canvas.height*0.4-s/2);
				Game.LeftBackground.globalAlpha=0.5;
			...
			if (Game.prefs.particles)
			{
				...
				for (var i=0;i<amount;i++)
				{
					...
					w*=-4;
					w+=Math.sin((n+Game.T*0.01)*Math.PI/2)*4;
					var x=0;
					var y=(140+n*16+w)-16;
					var r=(-(a)*360-Game.T*0.1);
			...
			Game.LeftBackground.drawImage(Game.Assets['shadedBorders.png'],0,0,Game.LeftBackground.canvas.width,Game.LeftBackground.canvas.height);
		};
	...
	Game.Logic=function()
	{
		...
		if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
		{
			...
			if (Game.cookiesEarned>=9999999999999 && Game.prestige['Heavenly chips']>=1) Game.Unlock('Caramoas');
			if (Game.cookiesEarned>=9999999999999 && Game.prestige['Heavenly chips']>=2) Game.Unlock('Sagalongs');
			if (Game.cookiesEarned>=9999999999999 && Game.prestige['Heavenly chips']>=3) Game.Unlock('Shortfoils');
			if (Game.cookiesEarned>=9999999999999 && Game.prestige['Heavenly chips']>=4) Game.Unlock('Win mints');
	...
	Game.MilkPics={0:'milkWave',1:'chocolateMilkWave',2:'raspberryWave',3:'orangeWave'};
